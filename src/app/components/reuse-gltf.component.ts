...
@Component({
  ...
  providers: [provideNgtObject(ShoeComponent), provideObjectRef(ShoeComponent), provideObjectHostRef(ShoeComponent)],
  imports: [NgIf, AsyncPipe, NgtGroup, NgtObjectPassThrough, NgtMesh, NgtValueAttribute, NgtMeshStandardMaterial]
})
export class ShoeComponent extends NgtObjectProps<THREE.Group> {
  ...
  template: `
    <ngt-soba-stage environment="city" intensity="0.6">
      <ng-template ngt-soba-stage-content>
        ...
        <sandbox-shoe color="orange" [rotation]="[0, 0.5, 180 | radian]" [position]="[0, 0, -2]"></sandbox-shoe>
      </ng-template>
    </ngt-soba-stage>
    <ngt-soba-orbit-controls autoRotate></ngt-soba-orbit-controls>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [NgtSobaStage, NgtSobaStageContent, ShoeComponent, NgtRadianPipe, NgtSobaOrbitControls]
})
